 ...
 /* $Id$ */
  ...
  if ($submit) {
    for ($i=0;$i<$lines;++$i) {
      ...
      if ( !strlen(trim(${$print_name})) ) {
	  $db->set_printer(${$print_id});
      } else {
        if (!${$print_top})  ${$print_top}  = 0;
        if (!${$print_left}) ${$print_left} = 0;
        if (!$db->set_printer(${$print_id},${$print_name},${$print_unit},${$print_top},${$print_left})) {
          $upd .= ",".${$print_name};
        }
      }
    }
