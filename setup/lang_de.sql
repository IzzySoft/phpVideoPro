...
INSERT INTO lang VALUES ('desc_backup_db','de','Ein Backup ist immer eine gute Sache (sicher ist sicher :). Für ein komplettes Backup sind zwar immer die Hausmittel der jeweiligen Datenbank (<code>mysql_dump</code>, <code>pgsql_dump</code>) vorzuziehen - ist jedoch z.B. kein Shell-Zugang zum Server verfügbar, läßt sich mit dem \"vollständigen Backup\" ein SQL-Backup-Script erstellen.<BR>Ist das Ziel hingegen eine Sicherung der Filmdaten (z.B. zur Duplizierung in eine andere Datenbank), empfiehlt sich eine Sicherung im phpVideoPro eigenen Format. Diese ermöglicht auch eine Migration der Daten z.B. von PostgreSQL zu MySQL. Und während nach dem Einspielen eines kompletten (SQL-) Backups einer älteren phpVideoPro Version in eine neuere wiederum ein Update der Datenbank nötig ist (als wäre phpVideoPro selbst gerade aktualisiert worden), sollte das \"PVP\" Format zwischen den Versionen kompatibel sein - die Zukunft wird\'s zeigen ;-)<BR>Ist die PHP-Installation auf dem Server mit Unterstützung für <code>gzip</code> kompiliert, kann durch ein Häkchen in der entsprechenden Box das Backup automatisch komprimiert werden. Die Backup-Datei wird damit um bis zu 90% kleiner ausfallen.<BR>','');
